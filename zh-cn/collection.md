...
When instantiating the object you can specify a second `bool` parameter, which will control the key searching in the object. 默认情况下`$insensitive`被设置为`true`，使得集合中的搜索不区分大小写。 Setting this value to `false` will make the search for the element in a case sensitive manner.
