...
[Phalcon\Mvc\View](api/phalcon_mvc#mvc-view) and [Phalcon\Mvc\View\Simple](api/phalcon_mvc#mvc-view-simple) are able to send events to an [Events Manager](events) if it is present. Events are triggered using the type `view`. If an event returns `false` it can stop the active operation. 以下事件被支持︰

| 事件名称               | 触发器                                 | Can stop |
| ------------------ | ----------------------------------- |:--------:|
| `afterRender`      | After completing the render process |    否     |
| `afterRenderView`  | After rendering an existing view    |    否     |
| `beforeRender`     | Before starting the render process  |    是的    |
| `beforeRenderView` | Before rendering an existing view   |    是的    |
| `notFoundView`     | When a view was not found           |    否     |
