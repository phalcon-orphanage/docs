...
| 类                                                             | 描述                                                                               |
| ------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| [Phalcon\Db\Adapter\Pdo\Mysql][db-adapter-pdo-mysql]      | 是世界上使用最多的关系数据库管理系统 (RDBMS)，作为提供多用户访问数量的数据库服务器运行                                  |
| [Phalcon\Db\Adapter\Pdo\Mysql][db-adapter-pdo-postgresql] | PostgreSQL 是一个功能强大的开源关系型数据库系统。 它有超过 15 年的积极发展和行之有效的体系结构，它赢得了良好声誉的可靠性、 数据完整性和正确性。 |
| [Phalcon\Db\Adapter\Pdo\Sqlite][db-adapter-pdo-sqlite]    | SQLite 是一个软件库，实现了一个自包含、 无服务器、 零配置、 事务性的 SQL 数据库引擎                                |
...
| `postgresql` | [Phalcon\Db\Adapter\Pdo\Mysql][db-adapter-pdo-postgresql] |
...
| 类                                                         | 描述                        |
| --------------------------------------------------------- | ------------------------- |
| [Phalcon\Db\Dialect\Mysql][db-dialect-mysql]           | SQL 特定方言为 MySQL 数据库系统的    |
| [Phalcon\Db\Dialect\Postgresql][db-dialect-postgresql] | SQL 特定方言 PostgreSQL 数据库系统 |
| [Phalcon\Db\Dialect\Sqlite][db-dialect-sqlite]         | SQLite 数据库系统的 SQL 特定方言    |
...
| `type`          | Column type. Must be a [Phalcon\Db\Column][db-column] constant (see below for a list)                                 |    否     |
...
| Index        | 描述                                                                                          | Optional |
| ------------ | ------------------------------------------------------------------------------------------- |:--------:|
| `columns`    | An array with columns defined with [Phalcon\Db\Column][db-column]                         |    否     |
| `indexes`    | An array with indexes defined with [Phalcon\Db\Index][db-index]                           |    是的    |
| `references` | An array with references (foreign keys) defined with [Phalcon\Db\Reference][db-reference] |    是的    |
