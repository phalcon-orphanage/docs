...
| 类                                                                            | 描述                                                                               |
| ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| [Phalcon\Db\Adapter\Pdo\Mysql](api/phalcon_db#db-adapter-pdo-mysql)      | 是世界上使用最多的关系数据库管理系统 (RDBMS)，作为提供多用户访问数量的数据库服务器运行                                  |
| [Phalcon\Db\Adapter\Pdo\Mysql](api/phalcon_db#db-adapter-pdo-postgresql) | PostgreSQL 是一个功能强大的开源关系型数据库系统。 它有超过 15 年的积极发展和行之有效的体系结构，它赢得了良好声誉的可靠性、 数据完整性和正确性。 |
| [Phalcon\Db\Adapter\Pdo\Sqlite](api/phalcon_db#db-adapter-pdo-sqlite)    | SQLite 是一个软件库，实现了一个自包含、 无服务器、 零配置、 事务性的 SQL 数据库引擎                                |
...
| 类                                                                        | 描述                        |
| ------------------------------------------------------------------------ | ------------------------- |
| [Phalcon\Db\Dialect\Mysql](api/phalcon_db#db-dialect-mysql)           | SQL 特定方言为 MySQL 数据库系统的    |
| [Phalcon\Db\Dialect\Postgresql](api/phalcon_db#db-dialect-postgresql) | SQL 特定方言 PostgreSQL 数据库系统 |
| [Phalcon\Db\Dialect\Sqlite](api/phalcon_db#db-dialect-sqlite)         | SQLite 数据库系统的 SQL 特定方言    |
...
When using numeric placeholders, you will need to define them as integers i.e. `1` or `2`. In this case `'1'` or `'2'` are considered strings and not numbers, so the placeholder could not be successfully replaced. With any adapter, data are automatically escaped using [PDO Quote](https://www.php.net/manual/en/pdo.quote.php). This function takes into account the connection charset, so its recommended to define the correct charset in the connection parameters or in your database server configuration, as a wrong charset will produce undesired effects when storing or retrieving data.
...
The adapters also send events to an [Events Manager](events) if it is present. If an event returns `false` it can stop the current operation. 以下事件被支持︰

| 事件名称                  | 触发器                                 | Can stop |
| --------------------- | ----------------------------------- |:--------:|
| `afterQuery`          | After a query is executed           |    否     |
| `beforeQuery`         | Before a query is executed          |    是的    |
| `beginTransaction`    | Before a transaction starts         |    否     |
| `createSavepoint`     | Before a savepoint is created       |    否     |
| `commitTransaction`   | Before a transaction is committed   |    否     |
| `releaseSavepoint`    | Before a savepoint is released      |    否     |
| `rollbackTransaction` | Before a transaction is rolled back |    否     |
| `rollbackSavepoint`   | Before a savepoint is rolled back   |    否     |
...
| `autoIncrement` | Set whether this column will be auto incremented by the database. Only one column in the table can have this attribute. |    是的    |
...
| `type`          | Column type. Must be a [Phalcon\Db\Column](api/phalcon_db#db-column) constant (see below for a list)                  |    否     |
...
| Index        | 描述                                                                                                         | Optional |
| ------------ | ---------------------------------------------------------------------------------------------------------- |:--------:|
| `columns`    | An array with columns defined with [Phalcon\Db\Column](api/phalcon_db#db-column)                         |    否     |
| `indexes`    | An array with indexes defined with [Phalcon\Db\Index](api/phalcon_db#db-index)                           |    是的    |
| `references` | An array with references (foreign keys) defined with [Phalcon\Db\Reference](api/phalcon_db#db-reference) |    是的    |
| `options`    | An array with creation options. (specific to the database system)                                          |    是的    |
