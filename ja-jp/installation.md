...
### PSR
Phalcon には PSR 拡張機能が必要です。 The extension can be downloaded and compiled from [this][psr-extension] GitHub repository. 拡張機能のインストール手順は、リポジトリの` README `に記載されています。 Depending on the installation method you chose, you might need to add a directive in your `php.ini` so that the PSR extension is loaded.
