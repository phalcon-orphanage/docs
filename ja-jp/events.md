...
## 命名規則
Phalconイベントは名前空間を使用して衝突を避けます。 Phalconの各コンポーネントは異なるイベント名前空間を占めており、自由に自分のものを作成できます。 イベント名は `component:event` としてフォーマットされます。 For example, as [Phalcon\Db][db] occupies the `db` namespace, its `afterQuery` event's full name is `db:afterQuery`.
