...
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |:--------:|
| `columns`    | An array with a set of table columns                                                                                                        |   いいえ    |
| `indexes`    | An array with a set of table indexes.                                                                                                       |    はい    |
| `references` | An array with a set of table references (foreign keys).                                                                                     |    はい    |
| `options`    | An array with a set of table creation options. These options are often related to the database system in which the migration was generated. |    はい    |
...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |:--------:|
| `type`          | Column type. Must be a [Phalcon\Db\Column](api/Phalcon_Db_Column) constant (see below)                                                   |   いいえ    |
| `size`          | Some type of columns like VARCHAR or INTEGER may have a specific size                                                                      |    はい    |
| `scale`         | DECIMAL or NUMBER columns may be have a scale to specify how much decimals it must store                                                   |    はい    |
...
| `default`       | Defines a default value for a column (can only be an actual value, not a function such as `NOW()`)                                         |    はい    |
...
| ------------------- | --------------------------------------------------------------------------------------------------- |:--------:| ---------------- |
| `referencedTable`   | It's auto-descriptive. It refers to the name of the referenced table.                               |   いいえ    | All              |
| `columns`           | An array with the name of the columns at the table that have the reference                          |   いいえ    | All              |
| `referencedColumns` | An array with the name of the columns at the referenced table                                       |   いいえ    | All              |
| `referencedSchema`  | The referenced table maybe is on another schema or database. This option allows you to define that. |    はい    | All              |
| `onDelete`          | If the foreign record is removed, perform this action on the local record(s).                       |    はい    | MySQL PostgreSQL |
| `onUpdate`          | If the foreign record is updated, perform this action on the local record(s).                       |    はい    | MySQL PostgreSQL |
