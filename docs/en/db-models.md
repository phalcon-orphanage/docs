...
- [Working with Models](#working-with)
    - [Creating Models](#creating)
        - [Public properties vs. Setters/Getters](#properties-setters-getters)
    - [Understanding Records To Objects](#records-to-objects)
    - [Finding Records](#finding-records)
        - [Model Resultsets](#resultsets)
        - [Filtering Resultsets](#filters)
        - [Binding Parameters](#binding-parameters)
    - [Initializing/Preparing fetched records](#preparing-records)
    - [Generating Calculations](#calculations)
    - [Creating/Updating Records](#create-update-records)
    ...
    - [Independent Column Mapping](#column-mapping)
    - [Record Snapshots](#record-snapshots)
    - [Pointing to a different schema](#different-schemas)
...
All the queries are internally handled as [PHQL](/en/[[version]]/db-phql) queries. PHQL is a high-level, object-oriented and SQL-like language. This language provide you more features to perform queries like joining other models, define groupings, add aggregations etc.
...
##### Bound parameters are available for all query methods such as `find()` and `findFirst()` but also the calculation methods like `count()`, `sum()`, `average()` etc. ##### {.alert .alert-warning}
...
##### Never use a `Phalcon\Db\RawValue` to assign external data (such as user input) or variable data. The value of these fields is ignored when binding parameters to the query. So it could be used to attack the application injecting SQL. ##### {.alert .alert-warning}
