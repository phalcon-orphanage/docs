...
オブジェクト変数は、`foo.bar` のような構文を使用してアクセスできる、属性を持つことができます。 配列を渡す場合、角括弧構文を使用する必要があります: `foo['bar']`
...
変数はフィルターを使用してフォーマットする事や、変更する事ができます。 パイプ演算子 `|` は、変数にフィルタを適用するために使用されます:
...
`{# ... #}</0>区切り文字を使用して、コメントをテンプレートに追加することもできます 。 それらの中のすべてのテキストは最終出力で無視されます:</p>

<pre><code class="twig">{# note: this is a comment
    {% set price = 100; %}
#}
`</pre> 
...
Voltは、リテラルや一般的な演算子を含む基本的な式のサポートを提供します。 式は、`{{;crwdn;ht;1;ht;crwdn; and ;crwdn;ht;2;ht;crwdn;}}` デリミタを使用して評価し、出力することができます:
...
テストでは、変数に有効な期待値があるかどうかをテストできます。 構文 `is` がテストを実行するために使用されます:
...
拡張テンプレートは他のテンプレートを拡張することができます。 次の例は、それを示しています:
...
フィルタは、テンプレート内に次の形式を持ちます: `leftExpr|name(optional-args)` 新しいフィルタを追加することは、関数で見られるのと似ています:
...
Voltを使うと、キャッシュビューの断片が簡単になります。 このキャッシングにより、ビューが表示されるたびにPHPのブロックの内容が実行されないように、パフォーマンスが向上します:
