...
<div class="alert alert-warning">
    <p>
        При использовании PHP 5.4/5.5 рекомендуется объявлять каждый столбец базы данных, который входит в модель в целях экономии памяти и уменьшения общего выделения памяти на выполнение.
    </p>
</div>
...
<div class="alert alert-warning">
    <p>
        Привязка параметров доступна для всех запросов метода, таких как f<code>find()</code> и <code>findFirst()</code>, а так же для методов <code>count()</code>, <code>sum()</code>, <code>average()</code> и т.д.
    </p>
</div>
