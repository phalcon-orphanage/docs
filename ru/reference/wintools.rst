...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Поскольку сценарии написаны на PHP, его необходимо установить на ваш компьютер. В зависимости от того как был установлен PHP его исполняемый файл может быть в разных местах. Найдите файл php.exe и запомните(скопируйте) путь к нему. Для примера, в последней редакции WAMP, PHP по умолчанию располагается в: *C:\\wamp\bin\\php\\php5.3.10\\php.exe*.
