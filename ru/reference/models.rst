...
:doc:`Phalcon\\Mvc\\Model <../api/Phalcon_Mvc_Model>` является родительским класс. Когда процесс выполняет несколько операций сом для всех моделей в вашем приложении Phalcon. Он обеспечивает независимость данных
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
При разработке мы можем встретиться с ситуацией, когда две идентичные записи происходят одновременно.
