...
для того, чтобы перемотать курсор в начало и получить запись в запрашиваемой позиции.
...
Обратите внимание, что наборы результатов могут быть сериализованы и храниться в кэше бэкэнда. :doc:`Phalcon\\Cache <cache>` может помочь с этой задачей.
Тем не менее, сериализация данных заставляет :doc:`Phalcon\\Mvc\\Model <../api/Phalcon_Mvc_Model>` получить все данные из базы данных в массив,
...
неправильная кодировка будет приводить к некорректному хранению и извлечению данных.
...
.. highlights::
    ...
    Поскольку тип связывания по умолчанию :code:`Phalcon\Db\Column::BIND_PARAM_STR`, нет необходимости указывать параметр "bindTypes", если все столбцы этого типа.
...
Методы :code:`belongsTo()`, :code:`hasOne()` или :code:`hasMany()` определяют отношения между одним или несколькими полями из текущей модели в поля другой модели.
Каждый из этих методов требует 3 параметра: поля текущей модели, модель, на которую ссылаются, и ее поля.
...
Первый параметр указывает локальные поля модели, используемые в отношениях; второй указывает имя модели; и третий - имя поля в указанной модели.
...
Если вызываемый метод имеет "get" префикс, то :doc:`Phalcon\\Mvc\\Model <../api/Phalcon_Mvc_Model>` вернет :code:`findFirst()`/:code:`find()`.
...
Префикс "get" используется для поиска связанных записей. В зависимости от типа отношений будет использоваться 'find' или 'findFirst':
...
-------------------------
По умолчанию отношения не ведут себя как внешние ключи базы данных, то есть, если вы пытаетесь вставить/обновить значение, не имея действительного значения
в модели, на которую ссылаетесь, то Phalcon не выведет никаких сообщений валидации. Вы можете изменить данное поведение, добавив четвертый параметр
...
:doc:`Phalcon\\Mvc\\Model <../api/Phalcon_Mvc_Model>` будет проверять, есть ли сеттеры, реализованные для столбцов, для значений переданных в массиве, отдавая приоритет им, вместо назначения значений непосредственно свойствам:
...
Пропуск столбцов
----------------
Можно указать :doc:`Phalcon\\Mvc\\Model <../api/Phalcon_Mvc_Model>` опускать некоторые поля при создании и/или обновлении записей для того,
...
.. highlights::
    ...
    Никогда не используйте :doc:`Phalcon\\Db\\RawValue <../api/Phalcon_Db_RawValue>` при вводе внешних данных (такие как ввод пользователя)
...
Кроме того, :doc:`Phalcon\\Mvc\\Model\\Behavior <../api/Phalcon_Mvc_Model_Behavior>` обеспечивает большую часть методов, необходимых,
