...
если 'format' - это строка, то она будет использоваться в качестве формата PHP функцией date_,  
...
Транзакции
------------
Когда процесс выполняет несколько операций базы данных, для поддержки целостности данных часто требуется,
чтобы каждый шаг был успешно завершен. Транзакции дают возможность обеспечить, чтобы все операции с базой 
данных были успешно выполнены прежде, чем данные зафиксируются в базе данных.
