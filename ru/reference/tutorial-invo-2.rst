...
чтобы плагины расширяли класс :doc:`Phalcon\\Mvc\\User\\Plugin <../api/Phalcon_Mvc_User_Plugin>`, но если они это делают, то упрощается доступ к сервисам приложения.
...
Создание списка ACL
^^^^^^^^^^^^^^^^^^^
В предыдущем примере мы получили ACL с помощью метода :code:`$this->getAcl()`. Этот метод реализуется в плагине.
