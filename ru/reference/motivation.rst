...
* Современные фреймворки используют так называемую ленивую или отложенную загрузку (автозагрузку) для увеличения производительности (для сценария используются только необходимые файлы с кодом)
...
Другие приложения генерируют контент практически сразу, изменяя от запроса к запросу. В этом случае PHP используется
