...
The adapters also send events to an [Events Manager](events) if it is present. If an event returns `false` it can stop the current operation. Podporovány jsou následující události:

| Jméno události        | Spuštění                            | Can stop |
| --------------------- | ----------------------------------- |:--------:|
| `afterQuery`          | After a query is executed           |    Ne    |
| `beforeQuery`         | Before a query is executed          |   Ano    |
| `beginTransaction`    | Before a transaction starts         |    Ne    |
| `createSavepoint`     | Before a savepoint is created       |    Ne    |
| `commitTransaction`   | Before a transaction is committed   |    Ne    |
| `releaseSavepoint`    | Before a savepoint is released      |    Ne    |
| `rollbackTransaction` | Before a transaction is rolled back |    Ne    |
| `rollbackSavepoint`   | Before a savepoint is rolled back   |    Ne    |
...
| `autoIncrement` | Set whether this column will be auto incremented by the database. Only one column in the table can have this attribute. |   Ano    |
...
| `type`          | Column type. Must be a [Phalcon\Db\Column](api/phalcon_db#db-column) constant (see below for a list)                  |    Ne    |
...
| ------------ | ---------------------------------------------------------------------------------------------------------- |:--------:|
| `columns`    | An array with columns defined with [Phalcon\Db\Column](api/phalcon_db#db-column)                         |    Ne    |
| `indexes`    | An array with indexes defined with [Phalcon\Db\Index](api/phalcon_db#db-index)                           |   Ano    |
| `references` | An array with references (foreign keys) defined with [Phalcon\Db\Reference](api/phalcon_db#db-reference) |   Ano    |
| `options`    | An array with creation options. (specific to the database system)                                          |   Ano    |
