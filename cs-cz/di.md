...
[Phalcon\Di](api/Phalcon_Di) is able to send events to an [EventsManager](events) if it is present. Events are triggered using the type 'di'. Některé události při vrácení hodnoty false můžou zastavit aktivní operaci. Podporovány jsou následující události:
...
| filter             | Input Filtering Service               | [Phalcon\Filter\FilterLocator](api/Phalcon_Filter_FilterLocator)                     |  Ano   |
...
| url                | URL Generator Service                 | [Phalcon\Url](api/Phalcon_Url)                                                        |  Ano   |
| viewCache          | Cache backend for views fragments     | None                                                                                   |   Ne   |
