...
## Events
[Phalcon\Mvc\View][mvc-view] and [Phalcon\Mvc\View\Simple][mvc-view-simple] are able to send events to an [Events Manager](events) if it is present. Events are triggered using the type `view`. If an event returns `false` it can stop the active operation. Podporovány jsou následující události:

| Jméno události     | Spuštění                            | Can stop |
| ------------------ | ----------------------------------- |:--------:|
| `afterRender`      | After completing the render process |    Ne    |
| `afterRenderView`  | After rendering an existing view    |    Ne    |
| `beforeRender`     | Before starting the render process  |   Ano    |
| `beforeRenderView` | Before rendering an existing view   |   Ano    |
| `notFoundView`     | When a view was not found           |    Ne    |
