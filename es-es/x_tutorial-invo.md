...
El siguiente formulario simple (`app/views/session/index.volt`) produce el HTML necesario para que los usuarios puedan enviar la información de inicio de sesión. Some HTML code has been removed to improve readability:
...
## Components
The UI has been created with the [Bootstrap][bootstrap] library. Algunos elementos, como la barra de navegación cambia según el estado de la aplicación. Por ejemplo, en la esquina superior derecha, el enlace `Log in / Sign Up` cambia a `Log out` si un usuario ha iniciado sesión en la aplicación.
