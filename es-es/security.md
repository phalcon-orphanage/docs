...
Hashes a string or password and returns the hashed string back. El segundo parámetro es opcional, y le permite establecer temporalmente un `factorTrabajo` específico o pasarlo, que sobreescribirá el predeterminado.
...
Getter and setter methods to specify the number of bytes to be generated by the openssl pseudo random generator. Por defecto es `16`.
