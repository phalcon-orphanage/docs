...
## Events
[Phalcon\Mvc\View][mvc-view] and [Phalcon\Mvc\View\Simple][mvc-view-simple] are able to send events to an [Events Manager](events) if it is present. Events are triggered using the type `view`. If an event returns `false` it can stop the active operation. Aşağıdaki olaylar desteklenmektedir:

| Olay Adı           | Tetiklendi                          | Can stop |
| ------------------ | ----------------------------------- |:--------:|
| `afterRender`      | After completing the render process |  Hayır   |
| `afterRenderView`  | After rendering an existing view    |  Hayır   |
| `beforeRender`     | Before starting the render process  |   Evet   |
| `beforeRenderView` | Before rendering an existing view   |   Evet   |
| `notFoundView`     | When a view was not found           |  Hayır   |
