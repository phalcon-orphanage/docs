...
The adapters also send events to an [Events Manager](events) if it is present. If an event returns `false` it can stop the current operation. Aşağıdaki olaylar desteklenmektedir:

| Olay Adı              | Tetiklendi                          | Can stop |
| --------------------- | ----------------------------------- |:--------:|
| `afterQuery`          | After a query is executed           |  Hayır   |
| `beforeQuery`         | Before a query is executed          |   Evet   |
| `beginTransaction`    | Before a transaction starts         |  Hayır   |
| `createSavepoint`     | Before a savepoint is created       |  Hayır   |
| `commitTransaction`   | Before a transaction is committed   |  Hayır   |
| `releaseSavepoint`    | Before a savepoint is released      |  Hayır   |
| `rollbackTransaction` | Before a transaction is rolled back |  Hayır   |
| `rollbackSavepoint`   | Before a savepoint is rolled back   |  Hayır   |
...
| `autoIncrement` | Set whether this column will be auto incremented by the database. Only one column in the table can have this attribute. |   Evet   |
...
| `type`          | Column type. Must be a [Phalcon\Db\Column](api/phalcon_db#db-column) constant (see below for a list)                  |  Hayır   |
...
| ------------ | ---------------------------------------------------------------------------------------------------------- |:--------:|
| `columns`    | An array with columns defined with [Phalcon\Db\Column](api/phalcon_db#db-column)                         |  Hayır   |
| `indexes`    | An array with indexes defined with [Phalcon\Db\Index](api/phalcon_db#db-index)                           |   Evet   |
| `references` | An array with references (foreign keys) defined with [Phalcon\Db\Reference](api/phalcon_db#db-reference) |   Evet   |
| `options`    | An array with creation options. (specific to the database system)                                          |   Evet   |
