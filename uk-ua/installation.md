...
### PSR
Phalcon потребує PSR-розширення. The extension can be downloaded and compiled from [this][psr-extension] GitHub repository. Інструкції з встановлення викладені у файлі `README` цього репозиторію. Depending on the installation method you chose, you might need to add a directive in your `php.ini` so that the PSR extension is loaded.
