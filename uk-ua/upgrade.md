...
# Оновлення до V4
Отже, ви вирішили оновитись до v4! **Congratulations**!!
...
## Вимоги
### PHP 7.2
Phalcon v4 підтримує тільки PHP 7.2 або вище. PHP 7.1 was released 2 years ago and its [active support][php-support] has lapsed, so we decided to follow actively supported PHP versions.

<a name='psr'></a>

### PSR
Phalcon потребує PSR-розширення. The extension can be downloaded and compiled from [this][psr-extension] GitHub repository. Інструкції з встановлення викладені у файлі `README` цього репозиторію. Після компіляції цього розширення у вашій системі його слід додати до `php.ini`. Ви маєте додати цей рядок:
...
### Встановлення
Download the latest `zephir.phar` from [here][zephir-phar]. Додайте її в теку, до якої буде мати доступ ваша система.
...
### Огляд
The `Phalcon\Filter` class has been rewritten to act as a service locator for different _sanitizers_. Цей об'єкт дозволяє вам знешкодити введені дані, як раніше використовувався метод `sanitize()`.
...
Для v4 ми переписали компонент, щоб реалізувати лише функціонал журналювання та приймання одного чи більше адаптерів, які відповідальні за виконання роботи з журналювання. This immediately offers compatibility with [PSR-3][psr-3] and separates the responsibilities of the component. Це також створює простий спосіб підключити понад одного адаптера до компонента логування, щоб дозволити журналювання подій декількох адаптерів. Використовуючи цю реалізацію, ми зменшили обсяг коду, необхідний для цього компонента та видалили старий компонент `Logger\Multiple`.
...
## Повідомлення
- `Phalcon\Messages\Message` and its collection `Phalcon\Messages\Messages` are new components that handle messages for models and validation. В минулому ми мали два компоненти, один для валідації та один для моделей. Ми об’єднали ці два компоненти, так щоб ви отримали `MessageInterface[]` під час виклику `save` у моделі або під час отримання повідомлень про результати валідації.
...
### Save
Метод `save()` більше не приймає параметри, щоб визначити дані. You can use `assign` instead.
...
Кешування View разом з сервісом `viewCache` було видалено з фреймворку, тому що вони несумісні з новим компонентом кешу. Developers can easily utilize a _view cache_ from external services such as Varnish, Cloudflare etc. Крім того, розробники можуть кешувати фрагменти, використовуючи `Phalcon\Mvc\View\Simple:render()` або `Phalcon\Mvc\View:toString()`. Ці два методи повертають згенерований HTML, який може бути кешований на сервері за вашим бажанням.
