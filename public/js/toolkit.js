...
$(document).on('click.bs.tab.data-api','[data-toggle="tab"]',clickHandler).on('click.bs.tab.data-api','[data-toggle="pill"]',clickHandler)}(jQuery);+function($){'use strict';var dataApi='[data-grid="images"]'
...
that.row=0
that.elements=[]
that.rownum+=1}if(that.images.length-1==index&&that.elements.length){that.resizeRow(that.row)
that.row=0
that.elements=[]
that.rownum+=1}})}
...
trackWidth+=fw
if(!isNotLast&&trackWidth<this.albumWidth){fw+=(this.albumWidth-trackWidth)}fw--
...
if(typeof option=='string')data[option].call($this)})}var old=$.fn.imageGrid
$.fn.imageGrid=Plugin
