...
# Atualizando para V4
Então você decidiu atualizar para a v4! **Congratulations**!!
...
## Requirements
### PHP 7.2
Phalcon v4 suporta apenas PHP 7.2 e superior. PHP 7.1 was released 2 years ago and its [active support][php-support] has lapsed, so we decided to follow actively supported PHP versions.

<a name='psr'></a>

### PSR
Phalcon requer a extensão PSR. The extension can be downloaded and compiled from [this][psr-extension] GitHub repository. As instruções de instalação estão disponíveis no arquivo `README` do repositório. Uma vez que a extensão tenha sido compilada e esteja disponível em seu sistema, você precisará carregá-la no seu `php.ini`. Você precisará adicionar essa linha:
