...
We will outline the areas that you need to pay attention to and make necessary changes so that your code can run as smoothly as it has been with v4. Embora as mudanças sejam significativas, são mais uma tarefa metódica do que uma tarefa assustadora.
...
### PSR
Phalcon requer a extensão PSR. The extension can be installed with PECL:

```bash
pecl install psr
```

If you do not wish to install it with PECL, you can download and compile OSR from [this][psr-extension] GitHub repository. As instruções de instalação estão disponíveis no arquivo `README` do repositório. Uma vez que a extensão tenha sido compilada e esteja disponível em seu sistema, você precisará carregá-la no seu `php.ini`. Você precisará adicionar essa linha:
...
Download the latest `zephir.phar` from [here][zephir-phar]. Adicione-o a uma pasta que pode ser acessada pelo seu sistema.
...
The [Assets](assets) component has had changes to the interface as well as some methods were renamed. A funcionalidade permanece a mesma que em versões anteriores.
