...
INVO was made with the client-side framework [Bootstrap][bootstrap]. Although the application does not generate actual invoices, it still serves as an example showing how the framework works.
...
For the sake of simplicity, we have used [sha1][sha1] to store the password hashes in the database, however, this algorithm is not recommended in real applications, use [bcrypt](security) instead.
...
Most options that manipulate data in INVO (companies, products and types of products) were developed using a basic and common [CRUD][crud] (Create, Read, Update and Delete). Each CRUD contains the following files:
