...
##### You must use a key length corresponding to the current algorithm. For the algorithm used by default it is 32 bytes. <br><br> If the algorithm for calculating the digest (signing) is not selected during the object construction, <strong>aes-256-cfb</strong> is selected by default. ##### {.alert .alert-warning}
