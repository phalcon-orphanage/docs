...
$manager
    ...
    // این جاوا اسکریپت ها در پایین صفحه قرار دارند
    ->collection('jsFooter')

    // نام خروجی نهایی
    ->setTargetPath('final.js')

    // برچسب اسکریپت با این URI تولید می شود
    ->setTargetUri('production/final.js')

    // این یک منبع از راه دور است که نیازی به فیلتر کردن ندارد
    ->addJs('code.jquery.com/jquery-1.10.0.min.js', false, false)

    // این منابع محلی هستند که باید فیلتر شوند
    ->addJs('common-functions.js')
    ->addJs('page-functions.js')

    // به تمام منابع در یک فایل واحد متصل شوید
    ->join(true)

    // از فیلتر Jsmin ساخته شده استفاده کنید
    ->addFilter(
        new Phalcon\Assets\Filters\Jsmin()
    )

    // از یک فیلتر سفارشی استفاده کنید
    ->addFilter(
...
// این جاوا اسکریپت ها در پایین صفحه واقع شده اند
...
| فیلتر                                                                 | توضیحات                                                                                                                  |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| [Phalcon\Assets\Filters\Jsmin](api/Phalcon_Assets_Filters_Jsmin)   | جاوا اسکریپت را با حذف کردن عوامل کاراکتر غیرضروری که توسط مترجمان جاوا اسکریپت/کامپایلرها نادیده گرفته شده، کوچک می کند |
| [Phalcon\Assets\Filters\Cssmin](api/Phalcon_Assets_Filters_Cssmin) | با پاک کردن عوامل کاراکتر غیرضروری که قبلا توسط مرورگرها نادیده گرفته می شد، CSS را کوچک سازی می کند                     |
...
<?php
/*
 * مسیرهای سفارشی را تعریف کنید.
 * این فایل در تعریف سرویس روتر تعریف می شود.
 */
...
class AssetsController extends ControllerBase
{
    public function serveAction() : Response
    {
        ...
        $contentPath = $this->assets->output($collection, function (array $parameters) {
            return BASE_PATH . "/ عمومی /". $parameters[0];
        }, $type);
...
location ~ ^/assets/ {
    ...
    # اگر فایل به عنوان یک فایل استاتیک وجود داشته باشد، آن را مستقیما بدون آن اجرا می کند
    # اجرای تمام تست های بازنویسی دیگر بر روی آن
